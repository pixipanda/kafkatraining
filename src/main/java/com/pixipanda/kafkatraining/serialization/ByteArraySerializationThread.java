 ...
 */
public class ByteArraySerializationThread implements Runnable {

    private final KafkaProducer<Long, byte[]> producer;
    private final String topic;

    public ByteArraySerializationThread(String brokers, String topic) {
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                ProducerRecord<Long, byte[]> record = new ProducerRecord<>(topic, key, value.getBytes());
    ...
    public static void main(String[] args) {
        ...
        String topic = args[0];
        ByteArraySerializationThread producerThread = new ByteArraySerializationThread("localhost:9092", topic);
